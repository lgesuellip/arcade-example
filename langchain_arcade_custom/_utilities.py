...
def create_tool_function(
    ...
    def tool_function(config: RunnableConfig, **kwargs) -> Any:
        ...
        # Business logic
        # 1. Get the default attendee for this user
        # 2. Get the user_id of arcade considering the phone number
        # 3. Authorize the user for the tool, it could be manage different depending on the api, etc

        from langchain_arcade_custom.logic import get_default_attendees, get_user_id, handle_tool_authorization

        if not kwargs.get("attendee_emails"):
            attendee_emails = get_default_attendees(config.get("configurable", {}).get("sender_id"))
            kwargs["attendee_emails"] = attendee_emails

        user_id = get_user_id(config.get("configurable", {}).get("sender_id")) if config else None

        if requires_authorization:
            auth_error = handle_tool_authorization(
                client=client,
                tool_name=tool_name,
                user_id=user_id,
                langgraph=langgraph
